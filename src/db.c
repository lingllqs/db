...
void *get_page(Pager *pager, uint32_t page_num) {
    ...
    if (pager->pages[page_num] == NULL) {
        ...
        if (page_num <= num_pages) {
            lseek(
                pager->file_descriptor, page_num * PAGE_SIZE, SEEK_SET
            );  // 跳过以加载的页
            ssize_t bytes_read =
                read(pager->file_descriptor, page, PAGE_SIZE);  // 加载一页内容
            if (bytes_read == -1) {
...
void serialize_row(Row *source, void *destination) {
    ...
    // USERNAME_SIZE); memcpy(destination + EMAIL_OFFSET, &(source->email),
    // EMAIL_SIZE);
